...
class ScrollReveal {
    ...
    setupScrollReveal() {
        ...
        // Dashboard Stats - Popup animation
        ...
        // Listing Cards - Alternate animations
        ...
        // Purchase Cards - Slide from left
        ...
        // Dashboard Nav Items - Slide from left
        ...
        // Settings Sections - Slide from bottom
        const settingsSections = document.querySelectorAll('.settings-section');
        settingsSections.forEach((section, index) => {
            section.classList.add('scroll-reveal-bottom');
            section.style.transitionDelay = `${index * 0.1}s`;
            observer.observe(section);
        });

        // User Profile Header - Popup
        const userProfileHeader = document.querySelector('.user-profile-header');
        if (userProfileHeader) {
            userProfileHeader.classList.add('scroll-reveal-popup');
            observer.observe(userProfileHeader);
        }

        // Tab Headers - Slide from right
        const tabHeaders = document.querySelectorAll('.tab-header');
        tabHeaders.forEach(header => {
            header.classList.add('scroll-reveal-right');
            observer.observe(header);
        });
    }

    addMagneticEffect() {
        const buttons = document.querySelectorAll('.btn-primary, .btn-secondary, .dashboard-nav-item');
        ...
        buttons.forEach(button => {
            button.addEventListener('mousemove', (e) => {
                ...
                button.style.transform = `translate(${deltaX * 8}px, ${deltaY * 8}px) scale(1.05)`;
            });
    ...
    addCardTiltEffect() {
        const cards = document.querySelectorAll('.listing-card, .stat-card, .purchase-card');
...
function addRippleEffect() {
    const selectors = '.btn-primary, .btn-secondary, .dashboard-nav-item, .listing-card, .stat-card, .icon-btn';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        console.log('🎨 Dashboard Animations Loaded!');
    }, 200);
});
...
document.querySelectorAll('.dashboard-nav-item').forEach(item => {
    item.addEventListener('click', () => {
        setTimeout(reinitializeAnimations, 300);
    });
});

// Observe grid changes
const grids = ['myListingsGrid', 'favoritesGrid', 'purchasesList'];
