...
class ScrollReveal {
    ...
    setupScrollReveal() {
        ...
        // Shop Toolbar - Fade in from top
        const toolbar = document.querySelector('.shop-toolbar');
        if (toolbar) {
            toolbar.classList.add('scroll-reveal-bottom');
    ...
    addMagneticEffect() {
        const buttons = document.querySelectorAll('.btn-primary-large, .product-add-btn, .btn-checkout, .category-filter');
...
function addRippleEffect() {
    const selectors = '.btn-primary-large, .product-add-btn, .btn-checkout, .category-filter, .product-card, .size-filter, .color-filter';
...
function setupFilterProductsOverride() {
    // Wait for the shop.js to load and define filterProducts
    const checkInterval = setInterval(() => {
        if (typeof window.filterProducts === 'function' && !originalFilterProducts) {
            ...
            console.log('✅ Filter products override installed');
        }
    }, 100);
...
document.addEventListener('DOMContentLoaded', () => {
    // CRITICAL: Make page visible immediately
    pageLoadAnimation();
    ...
    setTimeout(() => {
        ...
        console.log('🎨 Shop Page Animations Loaded!');
    }, 200);
});

// Re-observe when products grid changes
...
window.reinitializeShopAnimations = reinitializeAnimations;